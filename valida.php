	...
	if ((!empty($login)) AND (!empty($senha))) {
		...
		if($resultadoUsuario) {
			...
			if(password_verify($senha, $row_usuario['usu_senha'])){
				...
				$sql = "UPDATE tentativa_acesso SET tent_usu_id=? ORDER BY tent_id DESC LIMIT 1"; // add chave primária do usuário na última coluna adicionada;
				$tipo = "i";
				$param_id =  $_SESSION['usu_id'];

				preparaEnviaFecha($sql, $tipo, $param_id);

				if ($_SESSION['usu_tipo'] == 1 || $_SESSION['usu_tipo'] == 2) {

					header("Location: autentica.php");	//usuário master(1) não passa por a2f 
				}
				$_SESSION['numeroAleatorio'] = 4;
			}
			else{
